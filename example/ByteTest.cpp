...
{
    unsigned char  src[12] = { 0x11, 0x12, 0x13, 0x17, 0x18, 0x19, 0x23, 0x19, 0x12, 0x16, 0x90, 0xA9 };
    unsigned char* tmp     = src;
    u8             c       = GETVAR_8(tmp);
    u16            s       = GETVAR_LE16(tmp);
    u32            i       = GETVAR_LE32(tmp);
    u64            l       = GETVAR_LE64(tmp);
    printf("0x%x 0x%02x 0x%04x 0x%08x\n", c, s, i, l);

    bytes::Buffer binBuffer;
    ...
    unsigned char recvData[] = { 0x01, 0x03, 0x04, 0x00, 0x00, 0x00, 0x00, 0xFA, 0x33 };
    printf("modbus send crc 0x%04x\n", ModBusCRC16(sendData, sizeof(sendData)));

    int            recvLen = sizeof(recvData);
    unsigned char* pData   = recvData;
    // 作CRC校验
    unsigned short recvCrcCode = ((pData[recvLen - 2] & 0xff) << 8) | pData[recvLen - 1];
