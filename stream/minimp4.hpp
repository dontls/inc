...
// http://www.mp4ra.org/object.html 0xC0-E0  && 0xE2 - 0xFE are specified as
// "user private"
...
#define MP4E_SAMPLE_RANDOM_ACCESS                                              \
  1 // mark sample as random access point (key frame)
#define MP4E_SAMPLE_CONTINUATION                                               \
  2 // Not a sample, but continuation of previous sample (new slice)
...
int mp4_h26x_write_init(mp4_h26x_writer_t *h, MP4E_mux_t *mux, int width,
                        int height, int is_hevc);
void mp4_h26x_write_close(mp4_h26x_writer_t *h);
int mp4_h26x_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal,
                       int length, unsigned timeStamp90kHz_next);
...
int MP4D_open(MP4D_demux_t *mp4,
              int (*read_callback)(int64_t offset, void *buffer, size_t size,
                                   void *token),
              void *token, int64_t file_size);

/**
 *   Return position and size for given sample from given track. The 'sample' is
 * a MP4 term for 'frame'
 *
...
MP4D_file_offset_t MP4D_frame_offset(const MP4D_demux_t *mp4,
                                     unsigned int ntrack, unsigned int nsample,
 ...
 *       while (sps = MP4D_read_sps(mp4, num_of_avc_track, sps_count,
 * &sps_bytes))
 *       {
 ...
 *       }
 */
const void *MP4D_read_sps(const MP4D_demux_t *mp4, unsigned int ntrack,
                          int nsps, int *sps_bytes);
const void *MP4D_read_pps(const MP4D_demux_t *mp4, unsigned int ntrack,
                          int npps, int *pps_bytes);
 ...
 */
MP4E_mux_t *MP4E_open(int sequential_mode_flag, int enable_fragmentation,
                      void *token,
                      int (*write_callback)(int64_t offset, const void *buffer,
                                            size_t size, void *token));

/**
 *   Add new track
 *   The track_data parameter does not referred by the multiplexer after
 * function return, and may be allocated in short-time memory. The dsi member of
 ...
 *   The tracks numbered starting with 0, according to order of MP4E_add_track()
 * calls 'kind' is one of MP4E_SAMPLE_... defines
 *
 *   return error code MP4E_STATUS_*
 *
 *   Example:
 *       MP4E_put_sample(mux, 0, data, data_bytes, duration,
 * MP4E_SAMPLE_DEFAULT);
 */
...
/**
 *   Set SPS data. MUST be used for AVC (H.264) track. Up to 32 different SPS
 * can be used in one track.
 *
 *   return error code MP4E_STATUS_*
 */
...
/**
 *   Set PPS data. MUST be used for AVC (H.264) track. Up to 256 different PPS
 * can be used in one track.
 *
 *   return error code MP4E_STATUS_*
 */
...
/**
 *   Set or replace ASCII test comment for the file. Set comment to NULL to
 * remove comment.
 *
 *   return error code MP4E_STATUS_*
 */
...
enum {
  ...
  BOX_gnrv =
      FOUR_CHAR_INT('g', 'n', 'r', 'v'), // GenericVisualSampleEntryAtomType
  BOX_gnra =
      FOUR_CHAR_INT('g', 'n', 'r', 'a'), // GenericAudioSampleEntryAtomType
  ...
  // MP4 Atoms
  BOX_sdhd =
      FOUR_CHAR_INT('s', 'd', 'h', 'd'), // SceneDescriptionMediaHeaderAtomType
  BOX_dpnd = FOUR_CHAR_INT('d', 'p', 'n', 'd'), // StreamDependenceAtomType
  BOX_iods = FOUR_CHAR_INT('i', 'o', 'd', 's'), // ObjectDescriptorAtomType
  BOX_odhd =
      FOUR_CHAR_INT('o', 'd', 'h', 'd'), // ObjectDescriptorMediaHeaderAtomType
  ...
  BOX_tfdt =
      FOUR_CHAR_INT('t', 'f', 'd', 't'), // TrackFragmentBaseMediaDecodeTimeBox
  ...
  BOX_cgen = FOUR_CHAR_INT('\xa9', 'g', 'e',
                           'n'), // custom genre (as string or as byte!)
  ...
  BOX_stik = FOUR_CHAR_INT(
      's', 't', 'i', 'k'), // stik (byte)  0 = Movie   1 = Normal  2 = Audiobook
                           // 5 = Whacked Bookmark  6 = Music Video  9 = Short
  ...
  BOX_egid =
      FOUR_CHAR_INT('e', 'g', 'i', 'd'), // episode global unique ID (byte)
  BOX_desc = FOUR_CHAR_INT('d', 'e', 's', 'c'), // description
  BOX_clyr =
      FOUR_CHAR_INT('\xa9', 'l', 'y', 'r'),     // lyrics (may be > 255 bytes)
  ...
  // these from
  // http://lists.mplayerhq.hu/pipermail/ffmpeg-devel/2008-September/053151.html
...
typedef struct MP4E_mux_tag {
  ...
  int enable_fragmentation; // flag, indicating streaming-friendly
                            // 'fragmentation' mode
...
#else
    // as in ffmpeg
    0,   0,   0,   0x20, 'f', 't', 'y', 'p', 'i', 's', 'o',
    'm', 0,   0,   2,    0,   'm', 'p', '4', '1', 'i', 's',
    'o', 'm', 'i', 's',  'o', '2', 'a', 'v', 'c', '1',
#endif
};
...
*/
static unsigned char *minimp4_vector_alloc_tail(minimp4_vector_t *h,
                                                int bytes) {
  unsigned char *p;
...
*/
static unsigned char *minimp4_vector_put(minimp4_vector_t *h, const void *buf,
                                         int bytes) {
...
MP4E_mux_t *MP4E_open(int sequential_mode_flag, int enable_fragmentation,
                      ...
                      int (*write_callback)(int64_t offset, const void *buffer,
                                            size_t size, void *token)) {
  if (write_callback(0, box_ftyp, sizeof(box_ftyp),
                     token)) // Write fixed header: 'ftyp' box
    return 0;
  ...
  if (!mux->sequential_mode_flag) { // Write filler, which would be updated
                                    // later
    ...
    mux->write_pos +=
        16; // box_ftyp + box_free for 32bit or 64bit size encoding
  }
...
static const unsigned char *next_dsi(const unsigned char *p,
                                     const unsigned char *end, int *bytes) {
  if (p < end + 2) {
    *bytes = p[0] * 256 + p[1];
    return p + 2;
  } else
...
int MP4E_set_dsi(MP4E_mux_t *mux, int track_id, const void *dsi, int bytes) {
  ...
  return append_mem(&tr->vsps, dsi, bytes) ? MP4E_STATUS_OK
                                           : MP4E_STATUS_NO_MEMORY;
}

int MP4E_set_vps(MP4E_mux_t *mux, int track_id, const void *vps, int bytes) {
  track_t *tr = ((track_t *)mux->tracks.data) + track_id;
  assert(tr->info.track_media_kind == e_video);
  return append_mem(&tr->vvps, vps, bytes) ? MP4E_STATUS_OK
                                           : MP4E_STATUS_NO_MEMORY;
}
...
int MP4E_set_sps(MP4E_mux_t *mux, int track_id, const void *sps, int bytes) {
  track_t *tr = ((track_t *)mux->tracks.data) + track_id;
  assert(tr->info.track_media_kind == e_video);
  return append_mem(&tr->vsps, sps, bytes) ? MP4E_STATUS_OK
                                           : MP4E_STATUS_NO_MEMORY;
}
...
int MP4E_set_pps(MP4E_mux_t *mux, int track_id, const void *pps, int bytes) {
  track_t *tr = ((track_t *)mux->tracks.data) + track_id;
  assert(tr->info.track_media_kind == e_video);
  return append_mem(&tr->vpps, pps, bytes) ? MP4E_STATUS_OK
                                           : MP4E_STATUS_NO_MEMORY;
}
...
static int write_pending_data(MP4E_mux_t *mux, track_t *tr) {
  ...
  if (tr->pending_sample.bytes > 0 && tr->smpl.bytes >= sizeof(sample_t)) {
    ...
    assert(mux->sequential_mode_flag); // Separate atom needed for
                                       // sequential_mode only
    ...
    // Write data
    ERR(mux->write_callback(mux->write_pos, tr->pending_sample.data,
                            tr->pending_sample.bytes, mux->token));
...
static int add_sample_descriptor(MP4E_mux_t *mux, track_t *tr, int data_bytes,
                                 int duration, int kind) {
  sample_t smp;
  smp.size = data_bytes;
 ...
 */
static int mp4e_write_fragment_header(MP4E_mux_t *mux, int track_num,
                                      int data_bytes, int duration, int kind
  ...
  if (kind != MP4E_SAMPLE_CONTINUATION) {
    ...
    if (!mux->sequential_mode_flag) {
      ...
      if (tr->smpl.bytes < sizeof(sample_t))
        return MP4E_STATUS_NO_MEMORY; // write continuation, but there are no
...
#define TRACK_HEADER_BYTES 512
  ...
  for (ntr = 0; ntr < ntracks; ntr++) {
    ...
    index_bytes +=
        TRACK_HEADER_BYTES; // fixed amount (implementation-dependent)
    // may need extra 4 bytes for duration field + 4 bytes for worst-case random
    // access box
    index_bytes +=
        tr->smpl.bytes * (sizeof(sample_t) + 4 + 4) / sizeof(sample_t);
  ...
  if (!mux->sequential_mode_flag) {
    ...
    // Second is optional duration update at beginning of file in fragmentation
    // mode. This can be avoided using "till eof" size code, but in this case
    // indexes must be written before the mdat....
...
#define MOOV_TIMESCALE 1000
  ...
  // next_track_ID is a non-zero integer that indicates a value to use for the
  // track ID of the next track to be added to this presentation. Zero is not a
  // valid track ID value. The value of next_track_ID shall be larger than the
  // largest track-ID in use.
  WRITE_4(ntracks + 1);
  END_ATOM;
  ...
  for (ntr = 0; ntr < ntracks; ntr++) {
    ...
    ATOM(BOX_trak);
    ATOM_FULL(BOX_tkhd,
              7); // flag: 1=trak enabled; 2=track in movie; 4=track in preview
    WRITE_4(0);   // creation_time
    WRITE_4(0);   // modification_time
    ...
    // purposes). set mdia hdlr name field to what quicktime uses. Sony
    // smartphone may fail to decode short files w/o handler name
    if (handler_ascii) {
    ...
    if (tr->info.track_media_kind == e_audio ||
      ...
      if (tr->info.track_media_kind == e_audio) {
        ...
        WRITE_4((tr->info.time_scale
                 << 16)); // samplerate == = {timescale of media}<<16;
      }
  ...
  WRITE_1(size)
    ...
    if (tr->info.track_media_kind == e_video &&
      ...
      if (MP4_OBJECT_TYPE_AVC == tr->info.object_type_indication) {
        ...
        WRITE_2(0); // progressive, interlaced, non packed constraint, frame
                    // only constraint flags
        ...
        WRITE_1(3);      // ConstantFrameRate (2), NumTemporalLayers (3),
                         // TemporalIdNested (1), LengthSizeMinusOne (2)

        WRITE_1(3); // Num Of Arrays
        WRITE_1((1 << 7) |
                (HEVC_NAL_VPS & 0x3f)); // Array Completeness + NAL Unit Type
        WRITE_2(numOfVPS);
    ...
    ATOM_FULL(BOX_stts, 0);
    {
      ...
      for (i = 0; i < samples_count; i++, cnt++) {
        if (i == (samples_count - 1) ||
            sample[i].duration != sample[i + 1].duration) {
          WRITE_4(cnt);
    ...
    WRITE_4(0); // sample_size  If this field is set to 0, then the samples have
    ...
    // Sync Sample Box
    {
      ...
      if (ra_count != samples_count) {
        // If the sync sample box is not present, every sample is a random
        // access point.
...
#define MP4E_HANDLER_TYPE_MDIR 0x6d646972
    ...
    WRITE_4(0);
    WRITE_4(0); // reserved[3]
    WRITE_4(0); // name is a null-terminated string in UTF-8 characters which
                // gives a human-readable name for the track type (for debugging
...
static void h264e_bs_flush(bs_t *bs) { *bs->buf = SWAP32(bs->cache); }
...
static int find_mem_cache(void *cache[], int cache_bytes[], int cache_size,
                          void *mem, int bytes) {
  int i;
  if (!bytes)
    return -1;
 ...
 */
static int remove_nal_escapes(unsigned char *dst, const unsigned char *src,
                              int h264_data_bytes) {
...
static int patch_pps(h264_sps_id_patcher_t *h, bit_reader_t *bs, bs_t *bd,
                     int new_pps_id, int *old_id) {
  int bytes;
...
static void patch_slice_header(h264_sps_id_patcher_t *h, bit_reader_t *bs,
                               bs_t *bd) {
...
static int transcode_nalu(h264_sps_id_patcher_t *h, const unsigned char *src,
                          int nalu_bytes, unsigned char *dst) {
  int old_id;
  ...
  *dst = *src;
  ...
  case 7: {
    ...
    int id = find_mem_cache(h->sps_cache, h->sps_bytes, MINIMP4_MAX_SPS,
                            dst + 1, cb);
    if (id == -1)
      return 0;
    h->map_sps[old_id] = id;
  ...
  case 8: {
    ...
    int id = find_mem_cache(h->pps_cache, h->pps_bytes, MINIMP4_MAX_PPS,
                            dst + 1, cb);
    if (id == -1)
      return 0;
    h->map_pps[old_id] = id;
 ...
 *
 */
static const uint8_t *find_start_code(const uint8_t *h264_data,
                                      int h264_data_bytes, int *zcount) {
 ...
 */
static const uint8_t *find_nal_unit(const uint8_t *h264_data,
                                    int h264_data_bytes, int *pnal_unit_bytes) {
  const uint8_t *eof = h264_data + h264_data_bytes;
  int zcount;
...
int mp4_h26x_write_init(mp4_h26x_writer_t *h, MP4E_mux_t *mux, int width,
  ...
  tr.object_type_indication =
      is_hevc ? MP4_OBJECT_TYPE_HEVC : MP4_OBJECT_TYPE_AVC;
  tr.time_scale = 90000;
...
static int mp4_h265_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal,
                              int sizeof_nal, unsigned timeStamp90kHz_next) {
  int payload_type = (nal[0] >> 1) & 0x3f;
  int is_intra =
      payload_type >= HEVC_NAL_BLA_W_LP && payload_type <= HEVC_NAL_CRA_NUT;
  int err = MP4E_STATUS_OK;
 ...
 */
static uint32_t read_payload(MP4D_demux_t *mp4, unsigned nb,
                             boxsize_t *payload_bytes, int *eof_flag) {
  if (*payload_bytes < nb) {
    *eof_flag = 1;
...
/*
 *   Any errors, occurred on top-level hierarchy is passed to exit check: 'if
 * (!mp4->track_count) ... '
 */
#define ERROR(mess)                                                            \
  if (!depth)                                                                  \
...
int MP4D_open(MP4D_demux_t *mp4,
  ...
  struct {
    ...
    // kind of box children's: OD chunks handled in the same manner as name
    // chunks
    boxtype_t format;
  ...
  do {
...
#if MP4D_INFO_SUPPORTED
        {BOX_mdhd, 1, 1}, {BOX_mvhd, 1, 0}, {BOX_hdlr, 0, 0},
        {BOX_meta, 0, 0}, // Android can produce meta box without 'FullBox'
                          // field, comment this line to simulate the bug
#endif
#if MP4D_TRACE_TIMESTAMPS
        {BOX_stts, 0, 0}, {BOX_ctts, 0, 0},
#endif
        {BOX_stz2, 0, 1}, {BOX_stsz, 0, 1}, {BOX_stsc, 0, 1},
        {BOX_stco, 0, 1}, {BOX_co64, 0, 1}, {BOX_stsd, 0, 0},
        {BOX_esds, 0, 1} // esds does not use track, but switches to OD mode.
                         // Check here, to avoid OD check
    };

    // List of boxes, which contains other boxes ('envelopes')
    // Parser will descend down for boxes in this list, otherwise parsing will
    // proceed to the next sibling box OD boxes handled in the same way as atom
    // boxes...
    static const struct {
      uint32_t name;
      boxtype_t type;
    } g_envelope_box[] = {
        {BOX_esds, BOX_OD}, // TODO: BOX_esds can be used for both audio and
                            // video, but this code supports audio only!
        {OD_ESD, BOX_OD},
        {OD_DCD, BOX_OD},
        {OD_DSI, BOX_OD},
        {BOX_trak, BOX_ATOM},
    ...
    broken_android_meta_hack:
      ...
      if (box_bytes == 0 || // standard indication of 'till eof' size
          box_bytes == (boxsize_t)0xFFFFFFFFU // some files uses non-standard
                                              // 'till eof' signaling
      ) {
...
#else
        if (box_bytes) {
          ERROR("UNSUPPORTED FEATURE: MP4BoxHeader(): 64-bit boxes not "
                "supported!");
        }
        box_bytes = minimp4_read(mp4, 4, &eof_flag);
#endif
        ...
        if (box_name == g_fullbox[i].name) {
          ...
          if (box_name == BOX_meta) {
            if (FullAtomVersionAndFlags >= 8 &&
              ...
              depth++;
              stack[depth].bytes =
                  payload_bytes + 4; // +4 need for missing header
...
#if MP4D_TRACE_SUPPORTED
    box_path[depth] = (box_name >> 24) | (box_name << 24) |
                      ((box_name >> 8) & 0x0000FF00) |
                      ((box_name << 8) & 0x00FF0000);
    TRACE(("%2d  %8d %.*s  (%d bytes remains for sibilings) \n", depth,
           (int)box_bytes, depth * 4, (char *)box_path,
    ...
    if (depth) {
      ...
      if (box_bytes > stack[depth].bytes) {
        TRACE(("Wrong %c%c%c%c box size: broken file?\n",
               (box_name >> 24) & 255, (box_name >> 16) & 255,
               (box_name >> 8) & 255, box_name & 255));
    ...
    case BOX_stsc: // ISO/IEC 14496-12 Page 38. Section 8.18 - Sample To Chunk
                   // Box.
    ...
    case BOX_hdlr:
      if (tr) // When this box is within 'meta' box, the track may not be
              // avaialable
      {
        SKIP(4); // pre_defined
    ...
    case BOX_avcC: // AVCDecoderConfigurationRecord()
      // hack: AAC-specific DSI field reused (for it have same purpoose as
      // sps/pps)
    ...
    case OD_DCD:  // ISO/IEC 14496-1 Page 28. Section 8.6.5 -
    ...
    case OD_DSI:  // ISO/IEC 14496-1 Page 28. Section 8.6.5 -
                  // DecoderConfigDescriptor.
      assert(tr); // ensured by g_fullbox[] check
      if (!tr->dsi && payload_bytes) {
        MALLOC(unsigned char *, tr->dsi, (int)payload_bytes);
        for (i = 0; i < payload_bytes; i++) {
          tr->dsi[i] = minimp4_read(
              mp4, 1, &eof_flag); // These bytes available due to check above
        }
        tr->dsi_bytes = i;
        payload_bytes -= i;
        break;
    ...
    default:
      TRACE(("[%c%c%c%c]  %d\n", box_name >> 24, box_name >> 16, box_name >> 8,
             box_name, (int)payload_bytes));
    }
    ...
    if (box_name == BOX_trak) {
      ...
      void *mem =
          realloc(mp4->track, (mp4->track_count + 1) * sizeof(MP4D_track_t));
      if (!mem) {
      ...
      tr = NULL; // Avoid update of 'hdlr' box, which may contains in the 'meta'
                 // box
    }
 ...
 */
static int sample_to_chunk(MP4D_track_t *tr, unsigned nsample,
  ...
  *nfirst_sample_in_chunk = 0;
  ...
  for (nc = 0; nc < tr->chunk_count; nc++) {
    if (chunk_group + 1 <
            tr->sample_to_chunk_count // stuck at last entry till EOF
...
MP4D_file_offset_t MP4D_frame_offset(const MP4D_demux_t *mp4, unsigned ntrack,
...
static const void *MP4D_read_spspps(const MP4D_demux_t *mp4,
                                    unsigned int ntrack, int pps_flag, int nsps,
                                    int *sps_bytes) {
  ...
  if (pps_flag) {
    // Skip all SPS
    sps_count = p[bytepos++];
    skip_bytes = skip_spspps(p + bytepos,
                             mp4->track[ntrack].dsi_bytes - bytepos, sps_count);
    if (skip_bytes < 0)
      return NULL;
    bytepos += skip_bytes;
  }
  ...
  if (nsps >= sps_count)
    return NULL;
  skip_bytes =
      skip_spspps(p + bytepos, mp4->track[ntrack].dsi_bytes - bytepos, nsps);
  if (skip_bytes < 0)
    return NULL;
  bytepos += skip_bytes;
...
No|type|lng| duration           | bitrate| Stream type            | Object type
 ...
 2|vide|```| 104.12 s   2603 frm| 1960559| VisualStream           | Visual
ISO/IEC 14496-2   -  720x304 3|soun|ger| 104.06 s   2439 frm|  191242|
AudioStream            | Audio ISO/IEC 14496-3    -  6 ch 24000 hz 4|soun|eng|
104.06 s   2439 frm|  194171| AudioStream            | Audio ISO/IEC 14496-3 -
6 ch 24000 hz 5|subp|ger|  71.08 s     25 frm|       0| Forbidden              |
Forbidden 6|subp|eng|  71.08 s     25 frm|       0| Forbidden              |
Forbidden
...
No|type|lng| duration           | bitrate| Stream type            | Object type
 0|mdir|und|  92.42 s   3980 frm|  128000| AudioStream            | Audio
ISO/IEC 14496-3MP4 FILE: 1 tracks found. Movie time 92.42 sec
...
#define STR_TAG(name)                                                          \
  ...
  for (i = 0; i < mp4->track_count; i++) {
    ...
    printf("\n%2d|%c%c%c%c|%c%c%c|%7.2f s %6d frm| %7d|", i,
           (tr->handler_type >> 24), (tr->handler_type >> 16),
           (tr->handler_type >> 8), (tr->handler_type >> 0), tr->language[0],
           tr->language[1], tr->language[2],
           (65536.0 * 65536.0 * tr->duration_hi + tr->duration_lo) /
               tr->timescale,
           tr->sample_count, tr->avg_bitrate_bps);
    ...
    if (tr->handler_type == MP4D_HANDLER_TYPE_SOUN) {
      printf("  -  %d ch %d hz", tr->SampleDescription.audio.channelcount,
             tr->SampleDescription.audio.samplerate_hz);
    } else if (tr->handler_type == MP4D_HANDLER_TYPE_VIDE) {
      printf("  -  %dx%d", tr->SampleDescription.video.width,
